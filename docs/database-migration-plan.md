...
- User/Auth/User Profile tiếp tục giữ trong MongoDB.
- Dữ liệu nghiệp vụ chính chuyển sang PostgreSQL.
- PostgreSQL không tạo bảng `users`.
- Các bảng PostgreSQL có liên quan user chỉ lưu `mongo_user_id` dạng `varchar(24)` hoặc string tương đương.
- Không tạo foreign key từ PostgreSQL sang MongoDB.
...
| --- | --- | --- |
| `boards.members.userId` -> `users._id` | `members` là array object trong `boards`; `$lookup` sang `users` trong `getUsersBoard` | Bảng `board_members` với `mongo_user_id`; không FK sang `users` |
...
| `cards.assigneeId` -> `users._id` | ObjectId, service lookup qua `userModel.getManyByIds` | `cards.assignee_mongo_user_id`; không FK sang `users` |
...
- `cards.registeredBy` có dùng trong dữ liệu thật, nên map sang `cards.registered_by_mongo_user_id`.
...
## 4. Collection giữ MongoDB

| Collection | Giữ MongoDB | Lý do |
| --- | --- | --- |
| `users` | Có | Thuộc User/Auth/User Profile theo yêu cầu. Chứa email, password hash, username, displayName, avatar, isActive, verifyToken. |
| `counters` | Có, nếu vẫn phục vụ user code | Hiện chỉ dùng `increaseSeq("user_code")` để sinh `users.userCode`. Vì user giữ MongoDB, counter này nên tiếp tục đi cùng MongoDB. NestJS không cần thư viện riêng cho sequence này; có thể giữ logic atomic `findOneAndUpdate + $inc` trong MongoDB/auth module. |

## 5. Collection chuyển PostgreSQL
...
## 6. Mapping MongoDB collection -> PostgreSQL table
...
| `boards.members[]` | `board_members` | `userId` -> `mongo_user_id`; `role` -> enum `board_member_role`. |
| `columns` | `columns` | `boardId` -> `board_id`; `cardOrderIds` thay bằng `cards.position`; `statusColor` -> `status_color_id`. |
| `cards` | `cards` | `boardId`, `columnId`, `issueTypeId`, `versionId` thành FK; `assigneeId` -> `assignee_mongo_user_id`; `registeredBy` -> `registered_by_mongo_user_id`. |
...
## 7. Đề xuất master/reference data
...
## 8. Quy tắc lưu user_id từ MongoDB sang PostgreSQL

- Tất cả user reference trong PostgreSQL lưu bằng cột `mongo_user_id varchar(24)`.
- Không tạo bảng `users` trong PostgreSQL.
- Không tạo FK từ `mongo_user_id` sang MongoDB.
- Các field user hiện tại:
  - `boards.members.userId` -> `board_members.mongo_user_id`.
  - `cards.assigneeId` -> `cards.assignee_mongo_user_id`.
  - `cards.registeredBy` -> `cards.registered_by_mongo_user_id`.
- API/service khi cần profile user phải gọi MongoDB/auth service theo danh sách `mongo_user_id`, tương tự `userModel.getManyByIds` hiện tại.
- Nên validate format ObjectId 24 hex tại application layer hoặc DB check constraint, nhưng không coi đó là FK.
- Không cascade delete user sang PostgreSQL. Nếu user bị deactivate/delete ở MongoDB, dữ liệu PostgreSQL chỉ giữ user id lịch sử.

## 9. Thiết kế PostgreSQL đề xuất
...
- `deleted_at timestamptz null` cho bảng hiện có `_destroy` hoặc cần soft delete: `boards`, `columns`, `cards`, `issue_types`, `versions`, `board_members`.
...
- Index các truy vấn hiện có:
  - Board theo member: `board_members(mongo_user_id, board_id)`.
  - Card theo board/filter: `cards(board_id, deleted_at)`, `cards(column_id, position)`, `cards(assignee_mongo_user_id)`, `cards(issue_type_id)`, `cards(version_id)`, `cards(priority_id)`.
  - Search title/name có thể dùng trigram index sau nếu cần.

## 10. Risk khi dùng hybrid MongoDB + PostgreSQL

- Không có transaction atomic xuyên MongoDB và PostgreSQL; các flow vừa đổi user/auth vừa đổi nghiệp vụ cần outbox/saga hoặc retry rõ ràng.
- Không có FK bảo vệ `mongo_user_id`; dữ liệu có thể trỏ tới user đã xóa/deactivate.
- Join user profile phải làm ở application layer, tăng độ phức tạp và có thể gây N+1 nếu không batch.
- Migration cần map `_id ObjectId` sang UUID nhất quán; nếu mất mapping sẽ khó khôi phục quan hệ.
- Trong giai đoạn song song, API có thể phải đọc 2 nguồn dữ liệu, cần xác định source of truth cho từng module.
...
## 12. Verification path khi bắt đầu code
...
- Kiểm tra `board_members.mongo_user_id`, `cards.assignee_mongo_user_id`, `cards.registered_by_mongo_user_id` vẫn lookup được user MongoDB theo batch.
...
## 13. Những điểm đã chốt và còn cần xác nhận trước khi code

- ĐÃ CHỐT `cards.registeredBy` có tồn tại, map sang `cards.registered_by_mongo_user_id`.
- ĐÃ CHỐT `boards.members.role` lưu dạng số, migrate theo mapping `1 admin`, `2 pm`, `3 member`, `4 guest`.
- ĐÃ CHỐT giữ `counters` trong MongoDB nếu vẫn cần sinh `userCode`; không cần thư viện NestJS riêng.
- ĐÃ CHỐT dùng soft delete bằng `deleted_at` cho dữ liệu nghiệp vụ.
